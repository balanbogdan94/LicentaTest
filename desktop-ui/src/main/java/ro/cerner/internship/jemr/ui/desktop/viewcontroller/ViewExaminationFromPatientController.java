...
public class ViewExaminationFromPatientController implements Initializable {
		...
		try {
			...
			root = loader.load(
					getClass().getResource("/ro/cerner/internship/jemr/ui/desktop/viewcontroller/PacientWindow.fxml")
							.openStream());
			PacientWindowController pageController = (PacientWindowController) loader.getController();
			pageController.setCurrentPatient(patient);
	...
	public <T> void viewAnalysis(T event) {
		...
		{
			bpmLabel.setText("BPM: "+Double.toString(BitalinoData.calculateBPM(rawData,((double)rawData.size()/examinationTable.getSelectionModel().getSelectedItem().getFrequency()))));			
	...
	public void saveGraphic(ActionEvent event) {
		...
		try {
			root = loader.load(getClass()
					.getResource("/ro/cerner/internship/jemr/ui/desktop/viewcontroller/ViewExaminationFromPatient.fxml")
					.openStream());
		} catch (IOException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		ViewExaminationFromPatientController pageController = (ViewExaminationFromPatientController) loader.getController();
